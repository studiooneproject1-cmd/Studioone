...
export default function AddSliderForm({ action }: Props) {
  const router = useRouter();

  const [form, setForm] = useState<slidersData>({
    title: "",
    title_ar: "",
    description: "",
    description_ar: "",
  ...
  const handleFormSubmit = () => {
    startTransition(async () => {
      try {
        ...
        formData.append("button_name_ar", form.button_name_ar);

        formData.append("image", form.image ?? "");

        await action(formData);

        setToast({ message: "slider added successfully!", type: "success" });
        ...
        console.error(error);
        setToast({ message: "Failed to add slider.", type: "error" });
        setTimeout(() => setToast(null), 3000);
      }
    });
  };
  ...
  return (
    <main className="ml-3 xl:ml-7 mb-7">
      <div className="flex flex-col justify-start items-start border-b border-gray-500 w-[95vw] mb-7">
        <h1 className="text-lg md:text-2xl font-bold">Add New slider</h1>
      </div>
      ...
      >
        <Card className="w-full h-full">
          <CardHeader>
            <CardTitle>New slider Details</CardTitle>
            <CardDescription>
              Fill out the required fields below to create a new slider.
            </CardDescription>
          </CardHeader>
